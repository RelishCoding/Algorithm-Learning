...
再看其他情况。由状态转移方程 dp\[i][j] = max(dp\[i - 1][j], dp\[i - 1][j - weight[i]] + value[i]); 可以看出 i 是由 i-1 推导出来，那么 i 为 0的时候就一定要初始化。
...
dp\[i-1][j] 和 dp\[i - 1][j - weight[i]] 都在 dp\[i][j] 的左上角方向（包括正上方向），先遍历物品再遍历背包和先遍历背包再遍历物品，**虽然两个for循环遍历的次序不同，但是dp[i][j]所需要的数据就是左上角，根本不影响dp[i][j]公式的推导**！但先遍历物品再遍历背包这个顺序更好理解。
